...
To find a particular record faster, take advantage of the search options that let you filter the results. By default, you may search by `code`, by `catalog`, or by `code` + `catalog`.
The search filter may be collapsed as well.

When collapsed, it shows all applied filters within the current search.

If you want to add a new searchable field, use the dropdown.

[.center.text-left]
.Search filter
image::search-filter.png[Filter by code]
