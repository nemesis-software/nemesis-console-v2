    ...
    return (
      <Dropdown id="global-filter-sites" className="global-filter-dropdown">
        ...
        <Dropdown.Menu className="super-colors">
          <MenuItem className="live-edit-dropdown-item" active={'allSites' === this.state.selectedIndex} key={'allSites'} onClick={() => this.onSiteSelect(null, 'allSites')}>All sites</MenuItem>
          {this.state.sites.map((site, index) => {
            return <MenuItem className="live-edit-dropdown-item" active={index === this.state.selectedIndex} key={index} onClick={() => this.onSiteSelect(site, index)}>{site.name}</MenuItem>
          })}
        </Dropdown.Menu>
      </Dropdown>
    );
