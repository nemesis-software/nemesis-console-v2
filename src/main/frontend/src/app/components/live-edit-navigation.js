    ...
    return (
      ...
      <Dropdown id="live-edit-sites" className="live-edit-dropdown">
        <Dropdown.Toggle className="live-edit-toggle">
          <div className="live-edit-dropdown-content"><i className="fas fa-draw-polygon"/> Live edit <i
            className="material-icons arrow-icon">keyboard_arrow_down</i></div>
        </Dropdown.Toggle>
        <Dropdown.Menu className="super-colors">
          {this.props.sites.map((site, index) => {
            return <Dropdown.Item key={index} onClick={() => this.openLiveEditPage(site)}>{site.name['en'] ? site.name['en'].value : ''}</Dropdown.Item>
          })}
        </Dropdown.Menu>
      </Dropdown>
    )
